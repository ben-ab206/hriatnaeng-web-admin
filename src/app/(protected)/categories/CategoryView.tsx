...
import { NewCategoryType, SortData, SortKeys } from "./StaticTypes";
...
const CategoryView = () => {
  ...
  const {
    ...
    sort_by_created_at: sortData.created_at,
    sort_by_name: sortData.name,
  });
  ...
  const { mutateAsync: uploadImage, isPending: isImageUploadPending } =
    api.categories.uploadCategoryImage.useMutation();
  ...
  const onSort = (sort: OnSortParam) => {
    setSortData((prev) => {
      const newSortData: SortData = { ...prev };
      const { key } = sort as { key: SortKeys; order: string };

      // Assuming SortKeys is 'created_at' | 'name'
      // Use a type assertion to tell TypeScript that k is a valid key
      for (const k in newSortData) {
        if (k === key) {
          newSortData[k as keyof SortData] =
            newSortData[k as keyof SortData] === "asc" ? "desc" : "asc";
        } else {
          newSortData[k as keyof SortData] = "";
        }
      }
    ...
    try {
      ...
      if (values.id && values.id !== 0) {
        await updateCategory({
          ...
          parent_id:
            typeof values.parent_id === "number" ? values.parent_id : null,
