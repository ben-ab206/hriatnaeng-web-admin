...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  containerClassName = "h-[300px] w-[200px]",
  imageClassName = "max-h-[300px] max-w-full",
  ratio = "2:3",
  maxWidth,
  maxHeight,
}) => {
        ...
        >
          <div className="flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-xs text-gray-500">
              (Image with ratio {ratio},{" "}
              {allowedFileTypes.map((type) => type.split("/")[1]).join(" or ")})
            </p>
            <p className="text-xs text-gray-500">
              Max size {maxFileSize / 1000000}MB
            </p>
          </div>
          <input
            id="file-upload"
